...
const filter = undefined; //'for_ios'
...
export default class PixivApp<CamelcaseKeys extends boolean = true> {
    ...
    async waitReady() {
        if (!this.refreshToken) {
            throw new Error('[Error] No Set Refresh Token');
        }
        if (this.auth) {
            return true;
        }
    ...
    async refresh(token?: string) {
        ...
        this._lastRefresh = this._refresh(token).finally(() => {
            this._lastRefresh = null;
        });

        return this._lastRefresh;
    }
    private _lastRefresh: Promise<CamelcaseKeys extends true ? PixivClient : Pixiv_Client> | null;
    private async _refresh(
        ...
        const local_time = `${now_time.getUTCFullYear()}-${
            ...
            .getUTCMinutes()
            .toString()
            .padStart(2, '0')}:${now_time.getUTCSeconds().toString().padStart(2, '0')}+00:00`;
        ...
        try {
            axiosResponse = await instance.post(
        ...
        if (axiosResponse) {
            ...
            throw new Error('refresh Error 2');
        }
    }
    ...
    async login(
        ...
        if (typeof this.username !== 'string') {
            return Promise.reject(
                new TypeError(`Auth is required. Expected a string, got ${typeof this.username}`)
            );
        }

        if (typeof this.password !== 'string') {
            return Promise.reject(
                new TypeError(`Auth is required. Expected a string, got ${typeof this.password}`)
            );
        }
    ...
    userIllusts(
        id: number,
        params?: PixivParams
    ): Promise<CamelcaseKeys extends true ? PixivIllustSearch : Pixiv_Illust_Search> {
        params = {
            userId: id,
            type: 'illust',
            filter,
    ...
    illustDetail(
        id: number,
        params?: PixivParams
    ): Promise<CamelcaseKeys extends true ? PixivIllustDetail : Pixiv_Illust_Detail> {
        if (!id) {
    ...
    illustComments(
        id: number,
        params?: PixivParams
    ): Promise<CamelcaseKeys extends true ? PixivCommentSearch : Pixiv_Comment_Search> {
        if (!id) {
            return Promise.reject(new Error('illustId required'));
        }
        params = {
    ...
    searchAutoComplete(
        word: string
    ): Promise<CamelcaseKeys extends true ? PixivAutoComplete : Pixiv_Auto_Complete> {
        if (!word) {
            return Promise.reject(new Error('word required'));
        }
    ...
    illustBookmarkDetail(
        id: number,
        params?: PixivParams
    ): Promise<CamelcaseKeys extends true ? PixivBookmarkDetail : Pixiv_Bookmark_Detail> {
        if (!id) {
            return Promise.reject(new Error('illustId required'));
        }
        params = {
    ...
    userBookmarkTagsIllust(
        params?: PixivParams
    ): Promise<CamelcaseKeys extends true ? PixivBookmarkSearch : Pixiv_Bookmark_Search> {
        params = {
            restrict: 'public',
            ...params
        };
