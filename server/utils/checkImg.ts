...
export const isImgBufferComplete = function (szBuffer) {
    //png检查
    if (
        szBuffer[0] == 137 &&
        szBuffer[1] == 80 &&
        ...
        if (
            szBuffer[szBuffer.length - 5] == 68 &&
            szBuffer[szBuffer.length - 4] == 174 &&
            szBuffer[szBuffer.length - 3] == 66 &&
            szBuffer[szBuffer.length - 2] == 96 &&
            szBuffer[szBuffer.length - 1] == 130
        )
            return true;
        //有些情况最后多了些没用的字节
    ...
    } else if (szBuffer[0] == 255 && szBuffer[1] == 216) {
        //jpg
        //标准jpeg最后出现ff d9
        if (szBuffer[szBuffer.length - 2] == 255 && szBuffer[szBuffer.length - 1] == 217)
            ...
            for (var i = szBuffer.length - 2; i > szBuffer.length / 2; --i) {
                //检查有没有ffd9连在一起的
                if (szBuffer[i] == 255 && szBuffer[i + 1] == 217) return true;
            }
        }
    }
    return false;
